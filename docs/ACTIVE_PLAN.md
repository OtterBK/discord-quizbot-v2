...
**다음 세션은 그 문서의 "Phase 0 — 인프라 스켈레톤"부터 시작.**

**착수 전 처리할 네이밍 변경 TODO 2건** (2026-08-08 사용자 지시, `docs/WEB_INTEGRATION_PLAN.md`에도 기록):
1. "장바구니" → "퀴즈함"으로 용어 변경. 웹 UI 목업엔 이미 반영됐지만, 기존 디스코드 봇 코드
   (`omakase-quiz-room-ui.ts`/`quiz-info-ui.ts`/`omakase_components.ts`의 버튼 라벨·변수명·주석)엔
   아직 "장바구니"가 남아있음 — 디스코드 쪽 사용자 노출 문구까지 바꿀지 범위를 먼저 사용자와 확정할 것.
2. "오마카세 퀴즈" → "랜덤 퀴즈"로 용어 변경. 마찬가지로 웹 UI와 디스코드 쪽(`OmakaseQuizRoomUI` 클래스명,
   버튼 라벨, `quiz_info.title` 등) 둘 다 있음 — 클래스/변수명까지 리네임할지, 노출 문구만 바꿀지 범위를
   먼저 정할 것.
