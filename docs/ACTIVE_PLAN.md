...
>
> **멀티플레이 이식 가능성 조사 완료(2026-08-19, 코드 조사만 — 착수는 사용자 컨펌 대기 중)**: 프리셋
> CRUD 화면(자기 프리셋 조회/이름변경/항목제거/삭제)은 거의 그대로 재사용 가능하지만, 라이브 퀴즈함을
> 건드리는 부분은 그대로 이식 불가 — (1) 오마카세는 방장 판별이 멤버 단위(`room_owner`)인데 멀티는
> **길드 단위**(`this.readonly`: 방장 길드=false, 참가 길드=true)라 권한 체크 구조 자체가 다름, (2)
> 오마카세는 로컬 화면만 갱신하면 되지만 멀티는 참가 길드 전체에 IPC(`sendEditLobbySignal`,
> `CLIENT_SIGNAL.EDIT_LOBBY`)로 브로드캐스트해야 동기화됨, (3) 멀티 호스트 로비는 이미 컴포넌트 행이
> 5/5에 가까워 오마카세처럼 행을 줄이는 압축이 사실상 필수, (4) 참가(비방장) 길드엔 이미 별도 읽기전용
> 바구니 뷰어(`omakase_basket_readonly_select_menu`, 24개 상한)가 있어 정리/대체 필요. **다음 세션
> 후보**: ① AG섹션 실사용 재검증(버그 2건 재현 여부 포함), ② 사용자 컨펌 시 멀티플레이 이식 착수(위
> 3가지 구조적 차이를 반영한 재설계 필요, 단순 포팅 아님), ③ `DISCORD_UI_SEPARATION_AUDIT_PLAN.md`
> 기준 `quizbot/quiz_ui/` 전수 순회.
