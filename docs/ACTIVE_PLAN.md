...
> **✅ 2026-08-12 사용자가 다음 세션 착수 순서를 확정.** 아래 6개를 이 순서로 진행하기로 함(순서
> 자체가 확정 사항 — 바꾸려면 사용자에게 먼저 확인):
> 1. **[코드 수정] `basket_items` 소유권/공개여부 검사 추가** — B-7 항목 하단 "다음 세션 착수 전
>    확인할 것" 참고, 코드 조사는 이미 끝나 있어 바로 착수 가능.
> 2. **[결정 필요] B-1. 문제 일괄 등록** — 아래 B-1 항목, `docs/B1_BULK_IMPORT_EXPORT_TODO.md`의
>    3가지 결정부터.
> 3. **[사용자 논의 필요] 11. MMR 개선 / 12. 랜덤 추첨 알고리즘 평가** — 아래 B-2 항목 하단, 구체적
>    불만 사항부터 사용자에게 물어볼 것.
> 4. **[결정 필요] UI 개선 [P1] 남은 2건** — 아래 B-4 항목, `docs/UI_IMPROVEMENT_PLAN_ROUND2.md`.
> 5. **[결정 필요] UI 개선 [P2] 대표 항목들** — 같은 문서, 착수 순서는 사용자와 논의.
> 6. **[결정 필요] UI 개선 [P3]** — 같은 문서, 우선순위 가장 낮음.
>
> 1번만 코드 조사가 끝나 바로 착수 가능하고, 2~6번은 전부 "사용자와 먼저 논의/결정"이 선행돼야
> 함 — 순서대로 진행하되 각 항목 시작 전 반드시 확인할 것. 종합 배경은 `docs/
...
**🔴 다음 세션 최우선(2026-08-12 심화 조사 중 발견, 아직 미착수) — `basket_items`에 소유권/공개여부
검사가 없음**: SQL 인젝션은 막혔지만, `db_quiz.ts`의 `selectRandomQuestionListByBasket`은 여전히
`WHERE quiz_id = ANY($1::int[])`뿐 — `is_private` 필터도, 소유권 체크도 없음. 즉 다른 유저의 비공개
퀴즈 quiz_id를 정수로 알기만 하면(순차 발급이라 열거 가능) 그 문제들이 그대로 게임에 출제됨. **권장
수정**: 정당한 사용 흐름(`UserQuizSelectUI` 브라우징)에서 장바구니에 담기는 퀴즈는 항상
`loadUserQuizListFromDB(undefined)`가 반환하는 **공개(`is_private=false`) 퀴즈뿐**이므로,
`selectRandomQuestionListByBasket`의 CTE에 `and is_private = false` 조건을 추가하면 됨
(`selectQuizInfoById`가 이미 쓰는 것과 동일 패턴). 코드 조사는 여기까지 끝나 있음 — 회귀 테스트만
같이 추가하면 바로 착수 가능.
