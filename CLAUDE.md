...
- 운영 서버(`develop`/`master` 브랜치)에 신규 커밋을 반영할 땐 `auto_script/server_script/quizbot_update.sh`(신규) — `git fetch`+`reset --hard origin/<브랜치>` → `npm install` → `custom_node_modules` 패치 재적용 → `npm run build` → `systemctl restart`까지 한 번에 처리. `private_config.json` 등 gitignore 대상 파일은 안 건드림, `develop-claude`(작업용 브랜치)에서는 실행 거부되는 안전장치 있음.
