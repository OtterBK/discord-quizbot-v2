...
class UserQuizInfoUI extends QuizInfoUI
{
  ...
  constructor(user_quiz_info: any, readonly = true)
  {
  ...
  refreshUI() //ui에 quiz_info 재적용
  {
    ...
    else
    {
      ...
      let temp_question_select_menu: any = undefined;
  ...
  answerTypeToString(answer_type: any)
  {
  ...
  doQuizPlayEvent(interaction: any)
  {
    const user_quiz_info = this.user_quiz_info;
  ...
  doQuizEditorEvent(interaction: any)
  {
    //퀴즈만들기 통해서 왔을 경우임
    const user_quiz_info = this.user_quiz_info;
  ...
  editQuizInfo(modal_interaction: any)
  {
    const user_quiz_info = this.user_quiz_info;
  ...
  editTagsInfo(select_interaction: any)
  {
    const user_quiz_info = this.user_quiz_info;
