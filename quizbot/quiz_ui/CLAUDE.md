...
- **`user-question-info-ui.js`** — 유저 퀴즈의 문제 편집기. **파일 자체 주석: "건드릴 엄두가 안난다... 우선 돌아가면 장땡"** — 조심해서 다룰 것. 문제 최대 50개 제한. 이미지 URL 변경 시 `update()` 대신 강제 재전송(`sendDelayedUI(this, true)`) — Discord embed edit이 새 이미지 URL을 바로 안 불러오는 문제 우회.
