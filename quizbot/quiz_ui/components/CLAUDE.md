...
- **`base_components.js`** — 기본 퀴즈 UI(페이지네이션 버튼, 메인 화면, 서버 옵션 설정). `option_value_components`는 서버 설정 옵션별(9개) select row를 담은 객체, `createOptionValueComponents(option_name)`가 만듦 — 내부적으로만 쓰이지만(외부에서 직접 호출 안 함) export는 그대로 남아있음(의도적으로 정리 안 함, `docs/DUPLICATE_UI_PATTERNS.md` 참고).
...
**알려진 이슈**: `docs/DUPLICATE_UI_PATTERNS.md`에 이 5개 파일 전체를 대상으로 한 중복 UI 생성 로직 후보 10건이 기록돼 있음(번호 버튼 로우 반복, 동적 태그 select 루프 반복, 모달 보일러플레이트 반복 등) — 사용자 결정으로 **통합 작업은 보류 중**, 새로 손댈 때 참고만 할 것. 죽은 export였던 `note_ui_component`는 삭제됨(`docs/DEPRECATED_CODE_REMOVED.md`).
