...
- **`custom_quiz_components.js`** — 유저 제작 퀴즈(CRUD) 관련. `quiz_delete_confirm_comp`(일반 유저용, 취소/삭제 2버튼)와 `quiz_delete_confirm_admin_comp`(관리자 전용, 취소/삭제/삭제+영구밴 3버튼)가 같이 있음 — `user-quiz-info.ui.js`가 `interaction.user.id === PRIVATE_CONFIG.ADMIN_ID` 여부로 어느 걸 보여줄지 분기함. `admin_panel_comp`(밴 목록 관리/신고처리/퀴즈 관리 3버튼, `/quizmgr` 관리자 패널 메인 메뉴)도 여기 있음. `question_edit_comp2`엔 "현재 문제 복제" 버튼(B-3', 2026-08-07)이 추가됨(새 문제 추가/복제/삭제 3버튼). `question_preview_comp`(B-2, 2026-08-07 신설)는 이미지 재로드 + 문제용/정답용 오디오 미리듣기 2버튼 — 원래 `question_edit_comp`에 있던 "이미지 재로드" 버튼을 이쪽으로 옮긴 것.
...
**컴포넌트 export 개수 회귀 테스트**: `test/quiz_ui/components.test.js`가 `components.js`의 재수출 개수를 하드코딩된 숫자로 고정해둠(현재 62개) — 이 디렉터리에서 새 export를 추가/삭제하면 그 테스트의 숫자와 주석도 같이 갱신해야 함.
