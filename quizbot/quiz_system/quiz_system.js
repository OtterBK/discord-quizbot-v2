...
class MultiplayerQuizSession extends MultiplayerSessionMixin(QuizSession)
{
  ...
  async waitForSyncDone()
  {
    ...
    {
      await utility.sleep(50); //공정한 게임을 위해 제일 중요한 구간임
      ++wait_sync_done_time_count;

      if(wait_sync_done_time_count === 100) //5초
      {
        this.sendMessage({content:`\`\`\`🌐 다른 서버의 동기화 완료를 기다리는 중\`\`\``});
      }

      if(wait_sync_done_time_count === 400) //20초
      {
      ...
      if(wait_sync_done_time_count >= 900) //45초. 이정도면 그냥 뭔가 문제가 있음
      {
        this.syncFailed();
        return;
      }
