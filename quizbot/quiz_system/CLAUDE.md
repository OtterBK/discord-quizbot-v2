...
- **`initialize.js`** — `Initialize`(공통 setup) + 4개 하위 클래스(`DevQuiz`/`CustomQuiz`/`OmakaseQuiz`/`UnknownQuiz`). `generateHint`의 `base_answer.indexOf(rd_index) === ' '` 줄은 `rd_index`가 숫자라 사실상 항상 false로 보이는 의심스러운 로직(주석에 "그냥 해도 될 것 같다"고 저자 스스로 적어둠) — 손대기 전 `docs/BUGS_FOUND.md`에 기록할지 검토. dev 퀴즈는 **파일명 자체가 데이터**(`&^`로 저자 구분, `&#`로 복수 정답 구분) — 이 인코딩 바꾸면 기존 dev 퀴즈 파일 다 깨짐.
