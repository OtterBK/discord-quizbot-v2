...
`private_config.json`은 봇 토큰/DB 접속정보 등 민감한 값을 담고 있어 `.gitignore` 대상이라 저장소에
없습니다. 대신 뼈대 파일 `config/private_config.example.json`을 복사해서 채워 넣으면 됩니다.

```bash
cd /home/ubuntu/quizbot3
cp config/private_config.example.json config/private_config.json
vim config/private_config.json
```
